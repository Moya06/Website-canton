...
export const authMiddleware = (options = {}) => {
  const config = {
    redirectTo: '/login',
    publicRoutes: ['/', '/login', '/signup', '/public', '/api-docs', '/api'],
  ...
  return (req, res, next) => {
    ...
    const isPublic = config.publicRoutes.some(route => {
      ...
      // Starts with match (for API routes and subpaths)
      if (req.path.startsWith(route + '/')) return true;
      
      // Special case: allow POST to login/signup
      if ((req.path === '/login' || req.path === '/signup') && req.method === 'POST') return true;
      
      // Special case: allow POST to queja routes (complaints)
      if (req.path.startsWith('/queja/') && req.method === 'POST') return true;
